...
class ServiceProvider extends Database
{
    ...
    public function addServiceProvider($spName, $spEmail, $spContactNo, $spUsername, $spPassword, $spGender, $spAddressLine, $spDistrictId, $spTownId, $latitudeValue, $longitudeValue, $spQualification, $spSkills, $image, $serviceDescription, $keywords, $pricePackge, $status, $profileStatus)
    {
        $query = "INSERT INTO {$this->tableName} (name, email_address, contact_no, username, password, gender, address_line, district_id, 
        town_id, latitude_value, longitutde_value, qualification, skills, image, description, keywords, price, status, profile_status) 
        
        VALUES 

        (:name, :email_address, :contact_no, :username, :password, :gender, :address_line, :district_id, :town_id, :latitude_value, 
        :longitutde_value, :qualification, :skills, :image, :description, :keywords, :price, :status, :profile_status)";

        try {

            $statement = $this->connection->prepare($query);

            $statement->execute([':name' => $spName, ':email_address' => $spEmail, ':contact_no' => $spContactNo, ':username' => $spUsername, ':password' => $spPassword, ':gender' => $spGender, ':address_line' => $spAddressLine, ':district_id' => $spDistrictId, ':town_id' => $spTownId, ':latitude_value' => $latitudeValue, ':longitutde_value' => $longitudeValue, ':qualification' => $spQualification, ':skills' => $spSkills, ':image' => $image, ':description' => $serviceDescription, ':keywords' => $keywords, ':price' => $pricePackge, ':status' => $status, ':profile_status' => $profileStatus]);
    ...
    public function updateServiceProviderInfo($serviceProviderId, $spName, $spEmail, $spContactNo, $spUsername, $spPassword, $spGender, $spAddressLine, $spDistrictId, $spTownId, $latitudeValue, $longitudeValue, $spQualification, $spSkills, $image, $serviceDescription, $keywords, $pricePackge, $status, $profileStatus)
    {
        $query = "UPDATE {$this->tableName} SET name = :name, email_address = :email_address, contact_no = :contact_no, username = :username, password = :password, gender = :gender, address_line = :address_line, district_id = :district_id, town_id = :town_id, latitude_value = :latitude_value, longitutde_value = :longitutde_value, qualification = :qualification, skills = :skills, image = :image, description = :description, keywords = :keywords, price = :price, status = :status, profile_status: :profile_status WHERE service_provider_id = :service_provider_id";
