    ...
    <!-- <table class="[&>tbody>*:nth-child(even)]:bg-[#99767B] table border-2 border-[#6D2932] w-full text-center table-auto" id="sp-table">
        ...
        <tbody>
            <tr>
                ...
                <td class="p-3 text-center">District</td>
                <td class="p-3 text-center">Town</td>
                <td class="p-3 text-center">Qualification</td>
                <td class="p-3 text-center">Skills</td>
                <td class="p-3 text-center">Description</td>
                <td class="p-3 text-center">Keywords</td>
                <td class="p-3 text-center">Service Charge</td>
                <td class="p-3 text-center">Available state</td>
...
<script>
    $(document).ready(function() {
        ...
        showAllSpInAdminPanel();

        function showAllSpInAdminPanel() {
            $.ajax({
                url: '../ajax-file/ajax.php',
                type: 'POST',
                data: {
                    "request": 'showAllSpInAdminPanel'
                },
                success: function(response) {

                    // console.log(response);
                    $("#sp-table-container").html(response)
                    $("#sp-table").DataTable({
                        order: [
                            [2, "asc"]
                        ]
        ...
        $("body").on('click', '#update-sp-btn', function(e) {
            ...
            swalWithBootstrapButtons.fire({
                title: "Are you sure?",
                text: `Dear Admin! Are you sure to update ID No. ${spId}?`,
                icon: "warning",
                ...
                if (result.isConfirmed) {
                    ...
                    window.location.href = `/skill-wave-service-hiring-app/admin/edit-service-provider.php?serviceProviderId=${spId}`;
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            swalWithBootstrapButtons.fire({
                                title: "Deleted!",
                                text: `ID No. ${spId} detail has been deleted successfully.`,
                                icon: "success"
                            });
